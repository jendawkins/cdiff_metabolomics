dh = findall( 0, 'type', 'figure' );  % Find all figures
dh.delete               

X = load('all_data.mat');
targets = load('targets_dict.mat');
met_names = load('names.mat');
met_targets = load('met_targets_all.mat');
met_targs = cellstr(met_targets.a);

targets_all = targets.all_data_even;
data = X.a;
...
colors0 = cell(length(targets_all),1);
colors0(:) = {'b'};

carray0 = cellstr(targets_all);
...
colors1 = colors0;
carray = cellstr(targets.all_data);
